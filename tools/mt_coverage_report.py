...
TOPICS = [
    ...
    ("حالت‌های اجرا، انحراف مجاز و ریکوت",
     ["MT-CONC-0010", "MT5-PROC-0008", "MT4-PROC-0012", "MT-TRBL-0006", "MT-CMP-0010"]),
    ...
    ("ویرایش و بستن معامله (کامل و جزئی)",
     ["MT-PROC-0010", "MT4-PROC-0010", "MT-PROC-0023", "MT-CMP-0009", "MT-CMP-0012"]),
...
# MT4 <-> MT5 parity map. A one-sided area is only acceptable with a stated
# reason (the feature genuinely does not exist on the other platform).
PAIRS = [
    ...
    ("انحراف مجاز و نوع پر شدن سفارش", ["MT4-PROC-0012"], ["MT5-PROC-0008"], ["MT-CONC-0010"],
     "تنظیم Filling فقط در متاتریدر ۵ وجود دارد."),
    ...
    ("بستن معامله (کامل و جزئی)", ["MT4-PROC-0010"], [], ["MT-PROC-0010", "MT-CMP-0009"],
     "رفتار بستن جزئی در متاتریدر ۴ متفاوت است و رکورد جدا دارد."),
    ...
    ("چارت، تمپلیت، پروفایل و اشیاء", [], [],
     ["MT-PROC-0004", "MT-PROC-0005", "MT-PROC-0006", "MT-PROC-0007", "MT-PROC-0008"], ""),
    ...
    ("MetaEditor و زبان برنامه‌نویسی", [], [], ["MT-PROC-0018", "MT-CMP-0004"],
     "زبان دو پلتفرم متفاوت است و فایل‌ها جابه‌جا اجرا نمی‌شوند."),
    ("نصب، تنظیمات و پشتیبان‌گیری", [], [], ["MT-PROC-0001", "MT-PROC-0017", "MT-PROC-0020"], ""),
    ("خطاهای رایج پلتفرم", ["MT-TRBL-0014"], [], ["MT-TRBL-0001", "MT-TRBL-0002", "MT-TRBL-0003",
     "MT-TRBL-0004", "MT-TRBL-0005", "MT-TRBL-0006"], "خطای Trade context is busy مخصوص متاتریدر ۴ است."),
    ("سفارش‌های Stop Limit", [], ["MT5-CONC-0003"], [],
     "در متاتریدر ۴ وجود ندارد؛ پاسخ درست برای کاربر متاتریدر ۴ همین نبودن است."),
...
KNOWN_GAPS = [
    ...
    ("MetaTrader Market، Signals و کپی‌تریدینگ",
     "تصمیم آکادمی (D-0002) این است که هیچ محصولی پیشنهاد نشود و این پرسش‌ها به پشتیبانی انسانی "
     "ارجاع داده شوند — قواعد `ACA-RULE-0002` و `ACA-RULE-0003`. عمداً محتوای معرفی محصول ساخته نشده است."),
...
def main():
    ...
    # --- MT4 / MT5 parity ------------------------------------------------------
    parity_rows, asymmetric = [], 0
    for area, mt4_ids, mt5_ids, both_ids, note in PAIRS:
        def cell(ids):
            missing_here = [i for i in ids if i not in objects]
            ok = [i for i in ids if i in objects]
            text = "، ".join("`%s`" % i for i in ok) if ok else "—"
            if missing_here:
                text += " (ناموجود: %s)" % "، ".join("`%s`" % i for i in missing_here)
            return text, bool(ok)

        c4, has4 = cell(mt4_ids)
        c5, has5 = cell(mt5_ids)
        cb, hasb = cell(both_ids)
        if hasb or (has4 and has5):
            state = "✅"
        elif note:
            state = "ⓘ"
        else:
            state = "⚠️"
            asymmetric += 1
        parity_rows.append((state, area, c4, c5, cb, note or "—"))

    lines += ["", "## تناظر متاتریدر ۴ و ۵", "",
              "هر ناحیه باید برای هر دو پلتفرم پاسخ داشته باشد: یا رکورد جدا برای هر پلتفرم، "
              "یا یک رکورد مشترک. علامت ⓘ یعنی قابلیت واقعاً فقط در یک پلتفرم وجود دارد و دلیلش ذکر شده. "
              "علامت ⚠️ یعنی شکاف واقعی.", "",
              "| | ناحیه | متاتریدر ۴ | متاتریدر ۵ | مشترک | توضیح |",
              "|---|---|---|---|---|---|"]
    for row in parity_rows:
        lines.append("| %s | %s | %s | %s | %s | %s |" % row)
    lines += ["", "**نتیجه:** %d ناحیه بررسی شد؛ %d ناحیه دارای شکاف واقعی." % (len(parity_rows), asymmetric)]
    lines += ["", "> طبق قاعده‌ی `ACA-RULE-0001` مربی پیش از پاسخ به پرسش‌های اختصاصی پلتفرم باید بپرسد "
              "دانشجو از متاتریدر ۴ استفاده می‌کند یا ۵. برای رکوردهای ستون «مشترک» این پرسش لازم نیست."]
